    ...
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {!isOpen && (
        <button
          ...
          className="flex items-center justify-center w-16 h-16 bg-primary text-white rounded-full shadow-lg hover:bg-primary/90 transition-all duration-300 hover:scale-105"
      ...
      {/* Contenedor del chat */}
      {isOpen && (
        <div className="fixed bottom-4 right-4 w-[420px] h-[660px] bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-500 animate-in flex flex-col">
          {/* Header */}
          <header className="h-16 px-6 flex items-center justify-between bg-primary text-white">
            <div className="flex items-center gap-3">
              <Bot className="h-6 w-6" />
              <h1 className="text-xl font-semibold">Asistente Suncar</h1>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={startNewConversation}
                className="p-2 hover:bg-primary/80 rounded-lg transition-colors flex items-center gap-1"
          ...
          <div
            ...
            className="flex-1 h-[calc(100%-8rem)] p-6 overflow-y-auto bg-gray-50 relative"
          >
            ...
            {showSuggestions && !isLoading && messages.length <= 1 && (
              <div className="absolute left-0 right-0 bottom-28 px-6 animate-slide-up">
                  ...
                  <div className="p-2">
                    {displayedSuggestions.map((suggestion, idx) => (
                      <button
                        ...
                        className="w-full text-left p-2.5 hover:bg-primary/5 rounded-lg text-sm text-gray-700 transition-colors flex items-center gap-2 my-1"
                      >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100">
              <div className="flex gap-3 items-center">
                <textarea
                  ...
                  className="flex-1 h-12 py-2 px-4 rounded-full border border-gray-200 resize-none focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-gray-700 placeholder-gray-400 text-sm"
                />
                <button
                  onClick={sendMessage}
                  disabled={!input.trim() || isLoading}
                  className="p-3 bg-primary text-white rounded-full hover:bg-primary/90 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center shadow-md hover:shadow-lg"
                  aria-label="Enviar mensaje"
                >
                  {isLoading ? (
                    <svg className="h-5 w-5 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"></path></svg>
                  ) : (
      ...
      {showFeedbackModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-2xl w-[90%] max-w-md mx-4 overflow-hidden animate-modal-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              <p className="text-gray-700 mb-4">¿Por qué consideras que la respuesta no fue útil?</p>
              <div className="space-y-3">
                ...
                {selectedFeedbackOption === OPCIONES_FEEDBACK.length - 1 && (
                  <textarea
                    ...
                    className="w-full p-3 border border-gray-200 rounded-lg text-sm resize-none h-24 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent mt-3 animate-fade-in"
                  />
                )}
              </div>
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button
                  ...
                  className={clsx(
                    "px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors text-sm flex items-center gap-2",
