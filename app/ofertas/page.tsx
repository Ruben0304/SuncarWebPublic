...
function OfertasContent() {
  ...
  const [ofertasConDescuento, setOfertasConDescuento] = useState<
    OfertaSimplificada[]
  >([]);
  const [ofertasSinDescuento, setOfertasSinDescuento] = useState<
    OfertaSimplificada[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [sortBy, setSortBy] = useState<"precio-asc" | "precio-desc" | "nombre">(
    "precio-asc",
  );
  ...
  const [recommendationData, setRecommendationData] =
    useState<RecomendadorData | null>(null);
  ...
  useAOS({ duration: 450, once: true, easing: "ease-out" });
  ...
  useEffect(() => {
    if (showRecommendations && recommendationData) {
        ...
        filtered = filtered.filter((oferta) => {
          ...
          switch (priceFilter) {
            case "low":
              return precio < 10000;
            case "mid":
              return precio >= 10000 && precio < 50000;
            case "high":
              return precio >= 50000;
            default:
              return true;
      ...
      if (selectedBrandKey) {
        filtered = filtered.filter((oferta) =>
          matchesSelectedBrand(oferta.marca),
        );
      }
  ...
  const getCardAosDelay = useCallback(
    (index: number) => Math.min((index % 6) * 60, 300),
    [],
  );

  const navigateToOfertaDetalle = useCallback(
    (ofertaId?: string | null) => {
      if (!ofertaId) {
        return;
      }

      saveOfertasScrollPosition();
      router.push(`/ofertas/${ofertaId}`);
    },
    [router],
  );

  const handleOfertaCardKeyDown = useCallback(
    (event: KeyboardEvent<HTMLDivElement>, ofertaId?: string | null) => {
      if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        navigateToOfertaDetalle(ofertaId);
      }
    },
    [navigateToOfertaDetalle],
  );
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setRecommendationError(
          response.message || "Error al obtener recomendaciones",
        );
      }
    } catch (error) {
  ...
  return (
    <>
      <div
        className={
          isRestoringScroll ? "opacity-0 pointer-events-none select-none" : ""
        }
      ...
      >
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 pt-32 pb-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {loading && (
              <div
                className="flex items-center justify-center py-12 md:py-16 px-4"
                data-aos="fade-up"
              >
                        ...
                        <div className="bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 rounded-3xl p-8 md:p-12 text-center shadow-2xl relative overflow-hidden">
                          ...
                          <div className="relative z-10">
                            ...
                            <p className="text-lg md:text-xl text-white/95 max-w-3xl mx-auto leading-relaxed drop-shadow">
                              Aprovecha nuestros descuentos exclusivos con
                              reserva previa. ¡Entérate de nuestras promociones
                              y reserva ahora!
                            </p>
                            <Link
                              href="/contacto"
                              className="mt-6 inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-6 py-3 rounded-full border-2 border-white/30 hover:bg-white/30 transition-all duration-300 cursor-pointer group"
                            >
                              <Tag className="w-5 h-5 text-white group-hover:scale-110 transition-transform duration-300" />
                              <span className="text-white font-semibold">
                                Reserva Previa Disponible
                              </span>
                            </Link>
                          </div>
                        </div>
                      ...
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
                        {ofertasConDescuento.map((oferta, index) => (
                          <Card
                            ...
                            tabIndex={oferta.id ? 0 : -1}
                            aria-label={
                              oferta.id
                                ? `Ver detalles de ${oferta.descripcion}`
                                : undefined
                            }
                            onClick={() => navigateToOfertaDetalle(oferta.id)}
                            onKeyDown={(event) =>
                              handleOfertaCardKeyDown(event, oferta.id)
                            }
                          >
                ...
                <div className="max-w-3xl mx-auto space-y-3">
                  ...
                  <div className="flex items-start gap-3 bg-gradient-to-r from-amber-50 to-yellow-50 backdrop-blur border-2 border-amber-300 px-4 py-3 rounded-xl shadow-md text-xs sm:text-sm text-gray-800">
                    ...
                    <div className="leading-relaxed">
                      ...
                      <p>
                        Los precios expresados en USD son{" "}
                        <span className="font-semibold">
                          referenciales internacionales
                        </span>{" "}
                        y aplican para:
                      </p>
                      ...
                      <p className="mt-2 text-gray-700">
                        Para pagos en Cuba, los precios se cobrarán en{" "}
                        <span className="font-semibold">
                          monedas locales al tipo de cambio vigente
                        </span>
                        .
                      </p>
                    </div>
                  ...
                  <div className="flex items-start gap-3 bg-white/80 backdrop-blur border border-blue-100 px-4 py-3 rounded-xl shadow-sm text-xs sm:text-sm text-gray-600">
                    ...
                    <p className="leading-relaxed">
                      Los precios publicados son referenciales y pueden
                      ajustarse ligeramente (al alza o a la baja) según las
                      características finales de cada proyecto. Te avisaremos
                  ...
                  <div className="flex items-start gap-3 bg-gradient-to-r from-orange-50 to-yellow-50 backdrop-blur border border-orange-200 px-4 py-3 rounded-xl shadow-sm text-xs sm:text-sm text-gray-700">
                    ...
                    <p className="leading-relaxed">
                      ...
                      recomendamos hablar primero con nuestros profesionales
                      para recibir asesoría personalizada y obtener detalles
                      precisos sobre su proyecto solar.
                    </p>
                  </div>
                  <div className="flex items-start gap-3 bg-gradient-to-r from-green-50 to-emerald-50 backdrop-blur border border-green-200 px-4 py-3 rounded-xl shadow-sm text-xs sm:text-sm text-gray-700">
                    ...
                    <div className="leading-relaxed">
                      <p className="mb-2">
                        ...
                        Brindamos servicio garantizado en{" "}
                        <span className="font-semibold">
                          La Habana y sus municipios
                        </span>
                        .
                      </p>
                      <p className="text-gray-600">
                        Para otras provincias, existe disponibilidad según cada
                        caso. Le invitamos a contactarnos indicando su provincia
                        para evaluar las opciones de servicio y coordinar la
                  ...
                  <div className="flex items-start gap-3 bg-gradient-to-r from-purple-50 to-pink-50 backdrop-blur border border-purple-200 px-4 py-3 rounded-xl shadow-sm text-xs sm:text-sm text-gray-700">
                    ...
                    <p className="leading-relaxed">
                      Los presupuestos tienen una validez de{" "}
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
                      ...
                      <div className="flex-1">
                        <p className="text-gray-700 text-sm sm:text-base md:text-lg leading-relaxed">
                          ...
                          algunas ofertas puedes optarlas por
                          <span className="font-semibold text-blue-700">
                            {" "}
                            pago a plazo
                          </span>{" "}
                          a través de un familiar en España?
                        </p>
              ...
              >
                <Card className="max-w-md mx-auto shadow-lg">
                  <CardContent className="p-6 sm:p-8">
                    ...
                    <p className="text-sm sm:text-base text-gray-600 mb-6 leading-relaxed">
                      En este momento no tenemos ofertas disponibles. Vuelve
                      pronto para ver nuestras nuevas opciones.
                    </p>
                    <Button
                      asChild
                      className="bg-secondary-gradient hover:opacity-90 text-white text-sm sm:text-base px-4 sm:px-6"
            ...
            >
              <CardContent className="py-12">
                ...
                <h3 className="text-2xl font-bold mb-3">
                  ¿Interesado en alguna oferta?
                </h3>
                <p className="text-white/90 mb-8 max-w-lg mx-auto">
                  Contacta a nuestro equipo de especialistas para obtener más
